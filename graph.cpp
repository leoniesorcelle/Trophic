...
void Graph::connexite()
{
    ...
    colors.push_back(KAKI);
    colors.push_back(FUCHSIA);
    colors.push_back(ROSE);
    colors.push_back(VIOLET);
    colors.push_back(MARRON);
    ...
    int fortconnex = 0;
    for (auto &elt : m_vertices) {
      if (elt.second.m_interface->m_top_box.m_bg_color == -1) {
        for (int i = 0; i<elt.second.m_out.size(); i++) {
            //Premier degrès : m_vertices[m_edges[elt.second.m_out[i]].m_to]
            for (int j = 0; j<m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out.size(); j++) {
                //Deuxième degrés
                if (m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to == elt.second.m_interface->vertex_id) {
                    elt.second.m_interface->m_top_box.set_bg_color(colors[key]);
                    m_vertices[m_edges[elt.second.m_out[i]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                    fortconnex += 2;
                }
                for (int k = 0; k<m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out.size(); k++) {
                    //Troisième degrés
                    if (m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to == elt.second.m_interface->vertex_id) {
                        elt.second.m_interface->m_top_box.set_bg_color(colors[key]);
                        m_vertices[m_edges[elt.second.m_out[i]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                        m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                        fortconnex += 3;
                    }
                    for (int l = 0; l<m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to].m_out.size(); l++) {
                        //Quatrième degrés
                        if (m_edges[m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to].m_out[l]].m_to == elt.second.m_interface->vertex_id) {
                            elt.second.m_interface->m_top_box.set_bg_color(colors[key]);
                            m_vertices[m_edges[elt.second.m_out[i]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                            m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                            m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                            fortconnex += 4;
                        }
                        for (int m = 0; m<m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to].m_out[l]].m_to].m_out.size(); m++) {
                            //Cinquième degrés
                            if (m_edges[m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to].m_out[l]].m_to].m_out[m]].m_to == elt.second.m_interface->vertex_id) {
                                elt.second.m_interface->m_top_box.set_bg_color(colors[key]);
                                m_vertices[m_edges[elt.second.m_out[i]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                                m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                                m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                                m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[m_vertices[m_edges[elt.second.m_out[i]].m_to].m_out[j]].m_to].m_out[k]].m_to].m_out[l]].m_to].m_interface->m_top_box.set_bg_color(colors[key]);
                                fortconnex += 5;
                            }
                        }
                    }
    ...
    cout<<"Le nombre de sommets fortement connexes est :"<<fortconnex<<endl;
}
