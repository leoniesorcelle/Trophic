...
/// Cette classe est trop grosse, elle fait le repassage, r�pare la voiture, p�che au harpon...
...
class Widget
{
    protected :
        ...
        // Viewport du contenu (int�rieur Padding)
    ...
    /// M�thodes utilisables dans les classes d�riv�es
    /// et les classes qui ont un Widget ou d�riv� en attribut
    public :
        ...
        Widget(double x, double y, double w, double h) :
            m_frame(x, y, w, h) {}
        Widget() = default;
        // Destructeur virtuel en principe inutile tant que les objets ne sont pas d�truits polymorphiquement
        // ( ��d que des objets d�riv�s seraient delete en tant que Widget *, ce n'est pas le cas dans les exemples d'utilisation )
        // Note : il n'est pas utile de red�finir un destructeur virtuel par d�faut dans les classes d�riv�es (si celui par d�faut convient)
        virtual ~Widget() = default;

        /// Gestion familiale... navigation et �dition de l'arbre des sous-�l�ments...
        ...
        /// Les accesseurs de "styles" sont � compl�ter...
    ...
    /// En principe non directement utiles dans les classes d�riv�es (aspects techniques)
    private:
        // Gestion familiale... construction de l'arbre des �l�ments/sous-�l�ments ...
...
class WidgetBox : public Widget
{
    protected :
        ...
        // La WidgetBox reste dans le cadre de son parent (pr�f�rable)
        bool m_contained = true;

        // Position au moment du click quand on commence � bouger la WidgetBox
...
/// Diff�rents �l�ments de d�coration des fl�ches
/// Ceci peut �tre �tendu en �tendant le code WidgetEdge::draw
...
struct ArrowItem
{
    // Type de d�coration (pointe de fl�che, triangle, rond...
    ArrowItemType m_type;

    // Position relative de l'�l�ment le long de l'arc, 0 origine, 0.5 milieu, 1.0 destination
    double m_position;

    // Taille absolue de l'�l�ment (voir valeur par d�faut dans le constructeur)
    double m_size;

    // Proportion (pointe de fl�che plus aigue <1 ou plus obtuse >1) (voir valeur par d�faut dans le constructeur)
    double m_proportion;
...
class WidgetEdge : public Widget
{
    protected :
        ...
        /// Si les 2 pointeurs sont distincts du nul l'ar�te est trait�
        /// Les 2 instances point�es ne doivent pas �tre d�truites
        /// sous peine de plantage douloureux du programme !
        Widget *m_attach[2] = {nullptr, nullptr};

        // Elements de d�coration : pointes de fl�ches, rond etc...
